...
const BlockStrategy = {
  ...
  compute(actor) {
    ...
    const finalBase = toSafeNumber(
      (bestWeapon ? bestWeapon.system?.block?.final?.value : actor.system?.combat?.block?.final?.value) ?? 0
    );
    return withRules({
      type: "block",
      naturalBase,
...
const SupernaturalShieldStrategy = {
...
function computeEffectiveScore(candidate, attackData, defensesCounter) {
  ...
  const multiPenalty = candidate.applyMultipleDefensePenalty ? multipleDefensePenaltyFromAccumulated(accumulated) : 0;
  const projPenalty = isProjectileAttack(attackData) ? Number(candidate.projectilePenalty) || 0 : 0;
  return {
...
function pickBestDefenseCandidate(actor, { attackData = null, defensesCounter = null } = {}) {
  const candidates = [
