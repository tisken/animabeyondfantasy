...
function buildZeroDefenseResult({ actor, defenderToken, attackData }) {
  ...
  const defenseData = ABFDefenseData.builder().defenseAbility(0).armor(taFinal).inmodifiableArmor(false).defenseType("resistance").defenderId(actor.id).defenderTokenId(defenderToken?.id ?? "").weaponId("").shieldId("").stackDefense(false).applyMultipleDefensePenalty(false).projectilePenalty(0).build();
