...
async function _sendPsychicAttackToChat({
  ...
  await ABFAttackData.builder().attackAbility(roll.total).damage(Number(baseDamage) || 0).ignoreArmor(false).reducedArmor(0).armorType(power.system?.critic?.value ?? game.animabf.weapon.NoneWeaponCritic.NONE).damageType(game.animabf.combat.DamageType.NONE).presence(0).isProjectile(true).damagesEnergy(effectData?.affectsInmaterial?.value).automaticCrit(!!actor.system.general.modifiers.automaticCrit?.value).critBonus(0).critDamageBonus(actor.system.general.modifiers.critDamageBonus?.final?.value ?? 0).attackerId(actor.id).weaponId(power.id).targets(targets ?? []).build().toChatMessage({ actor, weapon: power });
}
