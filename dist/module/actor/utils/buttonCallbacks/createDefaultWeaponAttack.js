...
async function createDefaultWeaponAttack(sheet, e) {
  ...
  const attackData = ABFAttackData.builder().attackAbility(roll.total).damage(weapon.system.damage?.final?.value).ignoreArmor(weapon.system.ignoreArmor.value).reducedArmor(weapon.system.reducedArmor.final.value).armorType(weapon.system.critic?.primary?.value).damageType(game.animabf.combat.DamageType.NONE).presence(weapon.system.presence?.final?.value).isProjectile(false).automaticCrit(!!sheet.actor.system.general.modifiers.automaticCrit?.value).critBonus(0).critDamageBonus(sheet.actor.system.general.modifiers.critDamageBonus?.final?.value ?? 0).attackerId(sheet.actor.id).weaponId(weapon.id).build();
