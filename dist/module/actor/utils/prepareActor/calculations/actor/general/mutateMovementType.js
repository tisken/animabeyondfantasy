...
const mutateMovementType = (data) => {
  ...
  movementType.final.value = movementType.mod.value + data.characteristics.primaries.agility.final.value + Math.min(0, Math.ceil(data.general.modifiers.allActions.final.value / 20)) + armorsMovementRestrictions;
...
const mutateMovementDistances = (data) => {
  const { movementType } = data.characteristics.secondaries;
