...
class ABFActorSheet extends ActorSheetV1 {
  i18n;
  ...
  _setupDebouncedSheetUpdates(html) {
    this._pendingUpdate = {};
    this._flushPendingUpdate = this._flushPendingUpdate ?? foundry.utils.debounce(async () => {
      const flat = this._pendingUpdate;
      this._pendingUpdate = {};
      await this._applyFlatChanges(flat);
    }, 150);
  ...
  _activateContractibleButtons(html) {
    html.find(".contractible-button").click((e) => {
      ...
      const { ui: ui2 } = this.actor.system;
      ui2.contractibleItems = {
        ...ui2.contractibleItems,
        [contractibleItemId]: !ui2.contractibleItems[contractibleItemId]
      };
      this.actor.update({ system: { ui: ui2 } });
    });
  }
  ...
  async _onRoll(event) {
    ...
    if (dataset.roll) {
      ...
      let formula = dataset.rollPath ? `${this._getRollDieFormula(dataset.roll)} + ${rollValue} + ${mod ?? 0}` : `${dataset.roll}+ ${mod ?? 0}`;
  ...
  _resolveRollPathValue(element, path) {
    ...
    if (actorValue !== void 0) return actorValue;
    const dynamicItemMatch = String(path).match(
      /^system\.dynamic\.[^.]+\.(?<itemId>[^.]+)\.(?<itemPath>.+)$/
    );
    if (dynamicItemMatch?.groups) {
      const { itemId: itemId2, itemPath } = dynamicItemMatch.groups;
      const itemValue = foundry.utils.getProperty(this.actor.items.get(itemId2), itemPath);
      if (itemValue !== void 0) return itemValue;
    }
  ...
  buildCommonContextualMenu = (itemConfig) => {
    ...
    return new ContextMenuImpl(
      this.element instanceof HTMLElement ? this.element.querySelector(containerSelector) : this.element.find(containerSelector)[0],
      rowSelector,
      [...otherItems],
      ...isV14 ? [{ jQuery: false }] : []
    );
