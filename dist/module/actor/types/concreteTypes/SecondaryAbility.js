...
class SecondaryAbility extends Ability {
  ...
  static normalizeInflateInput(node) {
    ...
    if (out.attribute && typeof out.attribute === "object" && !Array.isArray(out.attribute) && "value" in out.attribute) {
  ...
  #calculateNaturalPenalty() {
    ...
    let naturalPenaltyReduction = Math.min(
      ...
      Math.floor(
        -naturalPenalty.unreduced.value * (this.maxNaturalPenaltyReductionPercentage / 100)
      )
    );
    let unreducedNaturalPenalty = naturalPenalty.final.value - naturalPenalty.reduction.value;
  ...
  #calculateNaturalMod(characteristicValue) {
    return Math.min(
      Math.max(
        ((this.attributeBonusAmount ?? 0) + 1) * calculateAttributeModifier(characteristicValue) + (this.naturalBonusAmount ?? 0) * 10,
        0
      ),
      100
    );
