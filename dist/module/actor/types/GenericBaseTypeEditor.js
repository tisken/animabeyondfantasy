...
function mergeEditorConfig(baseCfg, typeCfg) {
  ...
  const readonly = /* @__PURE__ */ new Set([...base.readonly, ...type.readonly]);
  const hidden = /* @__PURE__ */ new Set([...base.hidden, ...type.hidden]);
  ...
  for (const [fieldPath, o] of Object.entries(overrides)) {
    ...
    if (o.label !== void 0) labels[fieldPath] = String(o.label);
...
class GenericBaseTypeEditor extends FormApplication {
  ...
  getData() {
    ...
    const dynamicOptions = ctor?.getEditorFieldOptions?.(this.actor, { path: this.path }) ?? {};
  ...
  async _updateObject(_event, formData) {
    const update = {};
    const calcFromFormula = formData.calculateBaseFromFormula === "on" || formData.calculateBaseFromFormula === true;
    for (const f of this._fields) {
      if (f.readonly) continue;
      if (f.fieldPath === "base.value" && calcFromFormula) continue;
      const fullPath = `${this.path}.${f.fieldPath}`;
      if (f.kind === "bool") {
        update[fullPath] = formData[f.fieldPath] === "on" || formData[f.fieldPath] === true;
        continue;
      }
