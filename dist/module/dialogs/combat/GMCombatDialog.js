...
class GMCombatDialog extends FormApplication {
  ...
  prepareForCounterAttack(counterAttackBonus) {
    ...
    this.modalData.calculations = void 0;
    ...
    if (attacker.result && defender.result) {
      ...
      if (this.isDamagingCombat) {
        ...
        if (combatResult.canCounterAttack && (!projectile.value || distance.check || distance.enable && distance.value <= 1)) {
          ...
          const lifePoints = this.defenderActor.system.characteristics.secondaries.lifePoints.value;
          const automaticCrit = !!attacker.result.values?.automaticCrit;
          const isCritical = automaticCrit || lifePoints > 0 && damage / lifePoints * 100 >= 50;
  ...
  applyDamageSupernaturalShieldIfBeAble(supShieldId) {
    ...
    const { dobleDamage, immuneToDamage } = this.modalData.defender.supernaturalShield ?? DEFAULT_SUPERNATURAL_SHIELD_STATE;
