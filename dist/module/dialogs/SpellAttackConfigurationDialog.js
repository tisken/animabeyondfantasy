...
class SpellAttackConfigurationDialog extends FormApplication {
  ...
  async _sendAttack() {
    ...
    try {
      ...
      ).damageType(game.animabf.combat.DamageType.NONE).presence(0).isProjectile(true).automaticCrit(!!attacker.combat?.automaticCrit).critBonus(0).critDamageBonus(Number(attacker.combat?.critDamageBonus ?? 0)).attackerId(actor.id).weaponId(spell.id).targets(this.modalData.targets ?? []).build();
