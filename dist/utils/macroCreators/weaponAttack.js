...
const id = "weapon.attack";
async function createWeaponAttackMacro({ actor, item, slot }) {
  const actorUuid = actor.uuid;
  const itemUuid = item.uuid;
  const payload = { id, actorUuid, itemUuid };
  const command = `await game.animabf.macros.execute(${JSON.stringify(payload)});`;
  let macro = game.macros?.find(
    (m) => m.getFlag(System.id, "kind") === id && m.getFlag(System.id, "actorUuid") === actorUuid && m.getFlag(System.id, "itemUuid") === itemUuid
  );
  if (!macro) {
...
async function executor({ actor, item }) {
...
export {
  createWeaponAttackMacro as default,
  executor,
  id
};
