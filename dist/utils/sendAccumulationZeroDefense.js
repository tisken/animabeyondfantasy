...
import { ABFDefenseData } from "../module/combat/ABFDefenseData.js";
import { computeCombatResult } from "../module/combat/computeCombatResult.js";
...
async function sendAccumulationZeroDefense({
  defenderToken,
  attackerToken,
  attackData,
  messageId,
  ...
  const tokenUuid = storedTokenKey || defenderToken.document?.uuid || defenderToken.uuid || defenderToken.id || "";
  if (messageId) {
  ...
  const defenseData = ABFDefenseData.builder().defenseAbility(0).armor(taFinal).inmodifiableArmor(false).defenseType("resistance").defenderId(actor.id).defenderTokenId(defenderToken?.id ?? "").weaponId("").shieldId("").build();
