...
export async function resolveCritical({ baseCriticalValue, defenderActor, defenderTokenId }) {
  // 1. Attacker rolls 1d100 (no open roll) + damage
  ...
  if (finalCritLevel <= 0) {
    ...
    if (isLimb(location.zone)) {
      extraLines += finalCritLevel > 100
        ? `<p><strong>Miembro destrozado/amputado</strong></p>` : '';
    }
