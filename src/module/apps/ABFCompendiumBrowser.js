export default class ABFCompendiumBrowser extends Application {
  ...
  _getSubtitle(doc) {
    ...
    if (s.cost?.value !== undefined && s.cost.value !== 0) parts.push(`Coste: ${s.cost.value}`);
    if (s.benefit?.value !== undefined && s.benefit.value !== 0) parts.push(`Beneficio: ${s.benefit.value}`);
