...
2. Lanzar `npm run release`. La propia consola te va guiando:
   - Primero te pregunta si estás seguro de que quieres publicar ese número de versión, que te asegures de que no existe ya, etc... Para aceptar, pulsa Enter. Eso generará un archivo `animabf.zip` en la carpeta `animabeyondfantasy/package/` (que será creada también en ese momento).
